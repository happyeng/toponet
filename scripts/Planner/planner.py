...
class Planner:
    ...
    def gen(self, output, packet_space, ingresses, behavior_raw, fault_scenes=None):
        ...
        # add_dfy 设置loop
        if behavior["path"]["path_exp"][0] == behavior["path"]["path_exp"][-1]:
            dvnet.setLoop()
    ...
    def gen_all_pairs_loopfree(self, output):
        total_states = []
        self.isLoop = True
        
        for device1 in self.devices:
            states = self.gen(
                    None, device1, [device1],  r"(exist == 0, (`%s`.*`%s`) )" % (device1, device1)
            )
            
            if states:
                total_states.append((
                    device1, [device1], "exists == 0", "%s.*%s" % (device1, device1), states
                ))
        self.output_puml(total_states, output, True)
